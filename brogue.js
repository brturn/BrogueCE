...
var ASM_CONSTS = {
  873376: ($0) => { var nxt = window.keyOrMouseEvents.shift(); if(nxt){ _javascript_receiveNextKeyOrMouseEvent.apply(null, nxt); } else { _javascript_receiveNextKeyOrMouseEvent($0, 0, 0, 0, 0); } },  
 873556: () => { var origin = window.location.origin; window.keyOrMouseEvents = []; window.addEventListener('message', function(e){ if(origin!==e.origin) return; window.keyOrMouseEvents.push(e.data); }, false); },  
 873750: () => { return /^((?!chrome|android).)*safari/i.test(navigator.userAgent)?1:0; },  
 873821: () => { return Math.min(window.keyOrMouseEvents.length, 1); },  
 873877: ($0, $1, $2, $3, $4, $5, $6, $7, $8) => { if(!window.plotChars){ setTimeout(function(){ window.parent.postMessage(Object.values(window.plotChars), '*'); window.plotChars = null; },0); window.plotChars = {}; } window.plotChars[($1<<8)+$2] = ([$0,$1,$2,$3,$4,$5,$6,$7,$8]); }
};
